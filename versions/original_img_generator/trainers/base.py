    ...
    def train(self, num_epochs: int, device: torch.device,
        ...
        for index, epoch in pbar:
            for batch_idx, data in enumerate(dataloader):
                ...
                #     if not torch.isfinite(loss): #check of NaN hazard 
                #         print(f"🚨 NaN or Inf in loss at epoch {epoch}, batch {batch_idx}")
                #         print(f"Loss value: {loss.item()}")
                #         continue  # Skip the rest of the loop to avoid backward/update
        ...
        save_checkpoint(self.model, optimizer, scheduler=scheduler, scaler=None,
